...
function Outcome({ revealed, truth, range, resolvesAt }) {
  // An open question has no truth yet. Showing the pending state is the honest
  // rendering — there is no number to reveal until the election runs.
  const hasTruth = truth !== null && truth !== undefined
  const isRevealed = revealed && hasTruth

  const isInside = hasTruth && truth >= range.lo && truth <= range.hi
    ...
    <div className="relative mt-2 min-h-[86px] w-full">
      ...
      >
        <div
          ...
          style={{ transform: isRevealed ? 'scaleX(1)' : 'scaleX(0)' }}
        />
        <div
        ...
        <div
          className={`font-display text-truth font-bold text-amber transition-all duration-[320ms] ease-board ${
            isRevealed ? 'opacity-100 blur-0' : 'translate-y-3 opacity-0 blur-[4px]'
          }`}
        >
          {truth}
        </div>
    ...
    <>
      <div className="mt-7">
        <div className="mb-2.5 text-label font-semibold uppercase text-chalk-dim">
          Election {question.epoch}
          {question.index ? ` · question ${question.index} of ${question.of}` : ''}
        </div>
      ...
      <div className="flex min-h-[190px] flex-1 flex-col items-center justify-center gap-0.5 pb-2 pt-7 sm:max-h-[270px]">
        ...
        <Outcome
          ...
          range={range}
          resolvesAt={question.resolvesAtLabel ?? 'the next election'}
        />
      </div>
